...
class setting_config_multitask:
    """
    ...
    base_path = '/Users/arashghelman/Desktop/Masters/Thesis/'
    dataset_path = base_path + 'Datasets/'

    datasets = 'isic17'
    print("multi-task dataset ->", datasets)
    if datasets == 'isic18':
        data_path = dataset_path + 'isic2018/'
    elif datasets == 'isic17':
        data_path = dataset_path + 'isic2017/'
    elif datasets == 'PH2':
        data_path = dataset_path + 'PH2/'
    else:
    ...
    train_transformer = transforms.Compose([
        myNormalize_multi(datasets, train=True),
        myToTensor_multi(),
        myRandomHorizontalFlip_multi(p=0.5),
        myRandomVerticalFlip_multi(p=0.5),
        myRandomRotation_multi(p=0.5, degree=[0, 360]),
        myResize_multi(input_size_h, input_size_w)
    ])
    test_transformer = transforms.Compose([
        myNormalize_multi(datasets, train=False),
        myToTensor_multi(),
        myResize_multi(input_size_h, input_size_w)
    ])
