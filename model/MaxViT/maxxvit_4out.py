...
from model.MaxViT.lib.helpers import build_model_with_cfg, checkpoint_seq, named_apply
from model.MaxViT.lib.fx_features import register_notrace_function
from model.MaxViT.lib.layers import Mlp, ConvMlp, DropPath, ClassifierHead, trunc_normal_tf_, LayerNorm2d, LayerNorm
from model.MaxViT.lib.layers import create_attn, get_act_layer, get_norm_layer, get_norm_act_layer, create_conv2d
from model.MaxViT.lib.layers import to_2tuple, extend_tuple, make_divisible, _assert
from model.MaxViT.lib.registry import register_model
from model.MaxViT.lib.vision_transformer_relpos import RelPosMlp, RelPosBias  # FIXME move these to common location
...
if __name__ == '__main__':
    ...
    model = maxvit_rmlp_small_rw_224().cuda()
    # model = MaxxVit()
    print(model)
    input_tensor = torch.randn(1, 3, 224, 224).cuda()
    output = model(input_tensor)
    out_num = len(output)
    print('output lenth=', out_num)
    for i in range(out_num):
        print('out_{}.shape={}'.format(i, output[i].shape))
